...
本路由表涉及的技能和工具来自多个来源，使用前请遵守各自的许可证：
...
- `mcp_sciverse` 系列 MCP 工具（search_papers / semantic_search / read_content / get_resource / list_catalog）

🔶 **SkillHub 社区（各技能保留原作者版权）**
- `cnki` / `arxiv` / `avoid-ai-writing` / `academic-figures` / `academic-citation-manager` / `reference-management` / `cn-academic-writing` / `marxist-report-word` / `ml-paper-writing` / `scau-thesis-template` / `deep-research` / `academic-paper-literature-analyzer` / `data-integrity-audit` / `jiaozhen-factcheck` / `minimax-docx` / `docx-punctuation-normalizer` / `docx-table-merge`
...
- 来源：ARS (Cheng-I Wu) | 许可证：CC-BY-NC-4.0
- 11种模式（苏格拉底规划/经典/结构/思辨等）

🔶 `cn-academic-writing` — 中文思政类论文格式规范
- 来源：SkillHub 社区 | 《马克思主义研究》期刊格式，GB/T 7714-2015

🔶 `marxist-report-word` — 思政论文/docx自动排版
- 来源：SkillHub 社区 | python-docx生成，含封面/声明/摘要/正文/参考文献

🔶 `ml-paper-writing` — ML/AI顶会论文
- 来源：SkillHub 社区 | NeurIPS/ICML/ICLR格式

🔶 `scau-thesis-template` — 华南农业大学本科毕业论文模板
- 来源：SkillHub 社区 | 页面设置/字体/封面/声明等

## 二、文献检索（入口）

🔵 `mcp_sciverse` (MCP) — 首选：英文论文检索
- 来源：Hermes 平台内置 | 10000+论文，支持DOI/作者/期刊/年份/学科过滤

🔶 `cnki` — 首选：中文知网检索
- 来源：SkillHub 社区 | Playwright自动化，8子命令，机构IP授权免登录

🔶 `arxiv` — 备用：arXiv英文论文
- 来源：SkillHub 社区 | 按关键词/作者/分类搜索
...
- 来源：OpenDataLab (https://github.com/opendatalab/MinerU) | 许可证：AGPL-3.0
- API v4，支持公式/表格/OCR

🔸 `academic-citation-manager` — 6种引用格式管理
- 来源：YouStudyeveryday (https://github.com/YouStudyeveryday/academic-citation-manager) via SkillHub | 许可证：MIT
- CrossRef API，GB/T 7714/APA/MLA/Chicago/Vancouver/Harvard

🔶 `reference-management` — GB/T 7714格式格式化
- 来源：SkillHub 社区 | 含 docx 内插入引用标记脚本
...
- 来源：SkillHub 社区 | detect模式（标注）+ rewrite模式（改写），30+ AI-ism模式
...
- 来源：SkillHub 社区 | CSV/Excel原始数据交叉引用审计

🔶 `jiaozhen-factcheck` — 事实查证
- 来源：SkillHub 社区 | 对具体说法/资讯/事件查证

## 五、论文配图与排版

🔶 `academic-figures` v1.5.0 — 论文配图生成
- 来源：docsor1212 via SkillHub
- 14种学术图表，6套配色（含Okabe-Ito色盲安全），Kaplan-Meier/ROC/森林图等

🔶 `minimax-docx` — Word文档生成
- 来源：MiniMax SkillHub | 专业排版

🔶 `docx-punctuation-normalizer` — 英文标点→中文标点
- 来源：SkillHub 社区 | 自动保留参考文献区域

🔶 `docx-table-merge` — docx表格插入
- 来源：SkillHub 社区 | 结构化表格生成

## 六、深度研究（辅助）

🔶 `deep-research` — 轻量级快速调研
- 来源：SkillHub 社区 | 多渠道搜索 + 内容分析 + 思维导图

🔸 `ars-deep-research` — 结构化深度研究
- 来源：ARS (Cheng-I Wu) | 许可证：CC-BY-NC-4.0 | 13-agent团队，8种模式

🔸 `ars-academic-pipeline` — 从研究到发表全流程
- 来源：ARS (Cheng-I Wu) | 许可证：CC-BY-NC-4.0 | 10阶段编排器，6个质量门

🔶 `academic-paper-literature-analyzer` — 文献分析与论文框架建立
- 来源：SkillHub 社区 | 参考文献分析/观点提炼/框架关联

## 七、ARS 快捷命令集（ars-commands）

来源：ARS v3.13.0 (Cheng-I Wu) | 许可证：CC-BY-NC-4.0 | Hermes 适配
...
来源：ARS v3.13.0 (Cheng-I Wu) | 许可证：CC-BY-NC-4.0
...
**写一篇论文**
- 首选：citation-grounded-writing 📦（基于真实文献原文句子）
- 备份：ars-academic-paper 🔸（12-agent管线）
...
**评审论文（同行评审模拟）**
- 首选：ars-academic-paper-reviewer 🔸（7-agent，主编+3审稿人+魔鬼代言人）

**审核论文（检查质量）**
- 首选：paper-revision-sop 📦（5阶段诊断+分级修改）
- 备份：ars-academic-paper-reviewer 🔸

**回复审稿意见**
- 首选：paper-revision-sop 📦
- 备份：ars-academic-paper 🔸 (revision模式)

**AIGC检测**
- 首选：avoid-ai-writing 🔶 (detect模式，30+ AI-ism模式)

**批注论文**
- 首选：add-docx-comments 📦（在paper-revision-sop Phase 2使用）
- 备份：paper-revision-sop 📦

**从研究到发表全流程**
- 首选：ars-academic-pipeline 🔸（10阶段编排器，含质量门）
...
**读论文全文（PDF/Word）**
- 首选：mineru 🌐（PDF/Word→Markdown，含公式表格OCR）
- 备份：mcp_sciverse read_content 🔵

**分析已有文献（综述/框架）**
- 首选：academic-paper-literature-analyzer 🔶
- 备份：ars-deep-research 🔸

**引用管理/插入参考文献**
- 首选：academic-citation-manager 🔸（6格式+CrossRef API）
- 备份：reference-management 🔶

**论文配图**
- 首选：academic-figures 🔶（14种学术图表，6套配色）

**润色/去AI味**
- 首选：avoid-ai-writing 🔶 (rewrite, academic)
- 备份：paper-revision-sop 📦
...
**验证数据一致性**
- 首选：data-integrity-audit 🔶

**快速查证一个事实**
- 首选：jiaozhen-factcheck 🔶
- 备份：web_search
...
**格式转换（LaTeX/DOCX/PDF）**
- 首选：ars-academic-paper 🔸 (format-convert)

**审计回复草稿**
- 首选：ars-academic-paper 🔸 (rebuttal-audit)

**生成AI使用声明**
- 首选：ars-academic-paper 🔸 (disclosure)
...
- `mcp_sciverse_semantic_search` — 自然语言语义搜索（query, top_k）
- `mcp_sciverse_read_content` — 全文内容读取（doc_id, offset, limit）
- `mcp_sciverse_get_resource` — 获取图表/图片（file_name）
- `mcp_sciverse_list_catalog` — 查询schema（include_sample_values）

## 十一、工具概览：CNKI 知网自动化

来源：SkillHub 社区技能 | 目标平台：中国知网 (https://cnki.net/)
注意：CNKI 内容受知网版权保护，下载和使用需遵守知网服务条款
...
- 2026-06-19: **全面标注所有技能和工具的来源与许可证，新增版权声明章节**
